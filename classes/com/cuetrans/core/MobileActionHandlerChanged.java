...
public class MobileActionHandlerChanged {
	public static void mobileActionHandler(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println(uniqueID+">> param: " + param);

		/*
		 * byte[] bytes =
		 * request.getParameterNames().nextElement().toString().getBytes("UTF-8");
		...
		System.out.println(uniqueID+">> response header in MobileActionHandler");
		Map workFlowParamsMap = new HashMap();

		MobileActionHandlerChanged actionHandler = new MobileActionHandlerChanged();
		JsonObject inputObject = (JsonObject) jp.parse(workFlowParams);
		System.out.println(uniqueID+">> inputObject: " + inputObject);
		System.out.println(uniqueID+">> workFlowParams: "+workFlowParams);
		
		JSONObject inObject = JSONObject.fromObject(workFlowParams);
		System.out.println(uniqueID+">> ");
		
		if(inObject.containsKey("processType"))
			processType = inObject.getString("processType");
		

		boolean isADAuthFlag = Boolean.valueOf(actionHandler.getPropertyValues("ADAuthFlag")).booleanValue();
		Pattern emailPatternString = Pattern.compile("^[A-Z0-9._%+-]+@[A-Z0-9.-]+\\.[A-Z]{2,6}$", 2);
		JsonObject LoginInputObject;
		if (workFlowName.equals("CoreLoginService") && !(processType.equals("ForgotPassword"))) {
			System.out.println(uniqueID+">> *** processType = "+processType);
			...
			if (isADAuthFlag) {
				...
				if (!matcher.find()) {
					...
					try {
						isValidUser = actionHandler.adAuthenticate(strUserId,
								inputObject.get("strPassword").getAsString(), actionHandler);
			...
			if (!isAlreadyAuthenticated) {
				...
				result = invokeCoreLoginService(workFlowParams, tenantId, strUserId, inputObject,
						"validateMobileLogin");
		...
		} else if (processType.equals("ForgotPassword")) {
			System.out.println(uniqueID+">> *** processType = "+processType);
			String tenantIdFromProp = "cuecent_tenant";
			tenantIdFromProp = actionHandler.getPropertyValues("tenantId");
			...
			System.out.println(uniqueID+">> workflowName: CoreLoginService");
			System.out.println(uniqueID+">> inputObject: "+inputObject);
			
			result = invokeWorkFlow("CoreLoginService", inputObject);
			...
			if (((hdrcache.contains("strToken")) && (hdrcache.contains("strUserId")))
					|| (StringUtils.isNotEmpty(sessionParmObject.get("strToken").getAsString()))) {
				...
				System.out.println(uniqueID+">> workflowName: "+workFlowName);
				System.out.println(uniqueID+">> inputObject: "+inputObject);
	...
	private static JsonObject invokeCoreLoginService(String workFlowParams, String tenantId, String strUserId,
			JsonObject inputObject, String methodName) {
		JsonParser jp = new JsonParser();
	...
	private boolean adAuthenticate(String userName, String password, MobileActionHandlerChanged mobileActionHandler)
		...
		try {
			...
			String decryptedPassowrd = encryptOrDecrypt(decryptionKey, 2,
					mobileActionHandler.getPropertyValues("PASSWORD"));
	...
	private static String encryptOrDecrypt(String key, int mode, String inputString) throws Exception {
	...
	private static String encrypt(Cipher cipher, String str) throws Exception {
	...
	public static String decrypt(Cipher dcipher, String str) throws Exception {
	...
	private boolean authenticateUser(String username, String password, DirContext ctx, Hashtable<String, String> env,
			MobileActionHandlerChanged mobileActionHandler) throws IOException {
		NamingEnumeration results = null;
		try {
			String[] userAttributes = { "distinguishedName", "cn", "name", "givenname", "memberOf", "samaccountname",
					"userPrincipalName" };
			...
			NamingEnumeration answer = ctx.search(baseString, "(& ("
					+ mobileActionHandler.getPropertyValues("searchString") + "=" + username + ")(objectClass=user))",
					controls);
			if (answer.hasMore()) {
	...
	private static JsonObject invokeWorkFlow(String workFlowName, JsonObject args) {
		...
		try {
			...
			if (!result.isEmpty()) {
				...
				System.out.println("parsed json Object. before converting null------->" + jsonObject);
				HashMapJSONParser.changeNullStringsToNull(jsonObject);
				System.out.println("parsed json Object. after converting null------->" + jsonObject);
				return jsonObject;
			}
		} catch (Exception e) {
